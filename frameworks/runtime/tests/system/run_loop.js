...
test("Should wrap functions in a run loop when they are protected (via SC.protect())", function () {
  var MyExternalLibrary = {
    _asynchronousCallback: function () {
      //Binding of output of first object to input of second object
  		binding1 = SC.Binding.from("output", first).to("input", second).connect() ;
  ...
  // Wrap the function in a runLoop (in place).
  SC.protect(MyExternalLibrary, "_asynchronousCallback");
  MyExternalLibrary._asynchronousCallback();
